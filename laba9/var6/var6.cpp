...
int menu(void)
{
	...
	cout << "4. Поиск" << endl;
	cout << "5. Поиск минимального элемента" << endl;
...
//-----------------------------------------------------------
Address* findMin(char field[NAME_SIZE], Address** phead) // поиск минимального элемента
{
	Address* min = *phead;
	Address* t = *phead;
	...
	while (t) {
		if (strcmp(field, "name") == 0) {
			if (strlen(t->name) < strlen(min->name)) {
				min = t;
			}
		}
		t = t->next;
	...
	while (true)
	{
		...
		case 4: {	  char fname[NAME_SIZE];
			...
			find(fname, &head);
			  ...
			  break;
		case 5: {
			char field[NAME_SIZE];
			cout << "Введите поле (name): ";
			cin.getline(field, NAME_SIZE - 1, '\n');
			Address* min = findMin(field, &head);
			if (min) {
				cout << "Минимальный элемент: " << min->name << " " << min->city << endl;
			}
			else {
