...
//-----------------------------------------------------------
Address* findMax(char field[NAME_SIZE], Address** phead) // поиск максимального элемента
{
	Address* max = *phead;
	Address* t = *phead;
	while (t) {
		if (strcmp(field, "name") == 0) {
			if (strcmp(t->name, max->name) > 0) {
				max = t;
			}
		}
		t = t->next;
...
int main(void)
{
	...
	while (true)
	{
		...
		case 5: {
			...
			cin.ignore(cin.rdbuf()->in_avail());
			cin.sync();
			Address* max = findMax(field, &head);
			if (max) {
				cout << "Максимальный элемент: " << max->name << " " << max->city << endl;
			}
			else {
