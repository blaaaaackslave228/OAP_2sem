...
int main()
{
    ...
    do {
        ...
        cout << "7 - Сумма отрицательных чисел, последняя цифра которых равна 3." << endl;
        cout << "8 - Выход." << endl;
        cin >> choice;
        switch (choice)
        {
...
void sum()
{
    ...
    if (temp == NULL) {
        ...
        while (temp != NULL) {
            if (temp->value < 0 || temp->value % 10 == 3) {
                sum = sum + temp->value;
                found = true;
            }
            temp = temp->next;
        ...
        if (found) {
            cout << "Сумма элементов, меньших 0 и оканчивающихся на 3: " << sum << endl;
        }
        if (!found) {
...
void infile()
{
    ofstream fileone("D:/Денис/2 сем/оаип/лабы/files/lab6.7.txt");
...
void fromfile(list*& head)
{
    ...
    int count = 0;
    int number;
    ifstream filetwo("D:/Денис/2 сем/оаип/лабы/files/lab6.7.txt");
